...
OpenClaw picks a more radical path: **it does not try to sediment lessons at all, because it does not trust anyone to reliably decide what is worth sedimenting**. It argues that a sounder approach is to index every session's content thoroughly, then pair that index with a smart retrieval system so that the agent looks things up before acting and "learns" temporarily from what it finds. Put differently, self-improvement does not happen at a write moment; it happens at every retrieval moment.
