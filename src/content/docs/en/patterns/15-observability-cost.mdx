...
Second, **debug builds default to no reporting against production monitoring**. There is one critical line of code in here: if the build is in debug mode (i.e. a developer is locally compiling for testing), the exporter defaults to None. This is deliberate engineering discipline. Running tests during development tends to produce all kinds of weird metric spikes, and if that data leaks into the production monitoring dashboards, operators will be misled into thinking something is wrong in production.
...
OTel SDK has internal self-protection, but OpenClaw's 13 events are more complex (`tool.loop`, the "detecting agent itself stuck" event, may trigger more emits when emitted). OpenClaw's own `dispatchDepth < 100` protection is cleaner than wrapping OTel.
