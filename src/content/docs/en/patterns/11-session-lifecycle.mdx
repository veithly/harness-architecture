...
- **Memory substrate, UX ceiling**: how you start, persist, resume, and reset sessions directly determines whether users work continuously for days or re-explain for ten minutes every return.
- **Four session routes**: Codex appends JSONL rollouts with a `SessionMeta` first line plus a SQLite index, splitting `ThreadId` from `SessionId`; Claude Code spreads state across 22 files with 4 lifecycle events (startup / resume / clear / compact); OpenClaw keeps session as an identity in under 100 core lines; Hermes routes per-platform sessions via `SessionSource` with 4 reset modes.
- **Resume needs hidden premises**: replaying messages is not enough—cwd, approval mode, and git state never appear in the transcript and must be captured up front, as Codex's `SessionMeta` does.
- **Keep startup empty**: Claude Code's comment says "do not add ANY warmup logic"—extra work on the startup path is the trap earned through painful experience.
...
## How four systems manage sessions

### Codex · Treat session as a database engineering problem: JSONL persistence + SQLite index + Thread/Session ID separation

The starting point Codex makes about sessions is this: a session is the most important state container in an agent, so it should be engineered to database standards rather than treated as casual application state.
...
JSONL changes the failure mode: each line parses independently, so if a crash lands mid-write on line N, only line N is lost. Lines 1 through N-1 remain fully recoverable.
...
JSONL only appends one line (a few KB), one write syscall, and Codex measures < 5ms per turn write cost on a several-hundred-turn session.
...
Startup does not scan all files; only when a thread_id is missing from SQLite does it backfill once. This "files as durable layer + database as index layer" two-tier design is the standard pattern for database systems.
...
Claude Code does more on session than Codex, but with completely different reasoning.
...
Claude Code's experience shows 4 is the "minimum sufficient" granularity, since each source has clean do/don't lists making hook authoring more precise.

There is one line in the code comments that is critical engineering doctrine and deserves to be highlighted: **"do not add ANY 'warmup' logic.

It is CRITICAL that you do not add extra work on startup."** This rule comes from painful repeated experience.
...
Each one individually is reasonable (every PR adds 1-3 seconds, no reviewer would object), but a year later startup went from 2 seconds to 11 seconds.

CLI tools above 200ms feel "laggy" to humans, and 11 seconds completely breaks the "responsive tool" product positioning.

After enough pain, the team wrote this rule into the code comments so every new PR has to justify "why this work cannot be lazy".
...
## What session systems already agree on
...
## Separate identity, storage, and lifecycle
...
## Four session flows side by side
...
## Follow session entry points through source
