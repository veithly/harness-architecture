...
## Four loops, four failure models
...
**TOKEN_BUDGET soft verifier** (`query/tokenBudget.ts`): beyond hard exit conditions, there's a soft exit mechanism. Below 90% of budget, every turn nudges the model to continue; after 3 consecutive continues that each added under 500 tokens, the loop judges this as diminishing returns and proactively stops. The verifier becomes a "token-convergence-driven budget gatekeeper": if the model has gotten so verbose each iteration adds only 500 tokens of info, the task is effectively done, and continuing only wastes tokens.
...
OpenClaw is the only one of the four with the entire loop officially documented (`docs/concepts/agent-loop.md` lines 18-148), letting team members read docs to understand the loop (without reading source).
...
Hermes' starting point on agent loop is: long-running agents (a day, a week, a month) and short-running agents (one session) are completely different species.

Short-running agents pursue "this single task must be done right" (so need strong verifiers), long-running agents pursue "accumulate improvement across sessions" (so need memory + self-grading + skill self-learning).

Under this judgement, single loops don't need to be written complex, verifiers don't need to be hard-blocked. Single-loop crashes are OK; next run will be smarter due to memory.

Forcing hard verifiers makes the agent too rigid to handle long-running scenario diversity.
...
## What production loops already agree on
...
## My take: recovery before cleverness
...
## Build the smallest recoverable loop
...
<BuildRecipe
  ...
  avoid={[
    ...
    'Letting model self-rate as the sole verifier: the model often claims completion before actually completing the task (hallucination thinks it did the work); without external verifier backstop you will inevitably crash',
...
## Follow the main loop through source
...
Swapping them breaks things: Codex goals on a chatbot never converge. Hermes's 90-step cap on an IDE assistant cuts off mid-file. Claude Code's TOKEN_BUDGET on a coding agent may abort before tests pass.
...
Codex writes every step as append-only events (`rollout/event.rs` has 30+ event types, stored at `~/.codex/rollouts/<session_id>.json`). Recovery replays from start to rebuild turn state.

Pros: stable recovery; the stream also gives auditing, replay, analytics. Cons: every step writes to disk, multi-MB after a long session; non-idempotent events (timestamps) must be filtered on replay.
...
**Follow-up**: "Does the judge agent need a TOKEN_BUDGET soft exit?" Yes, but with different numbers. Judges prefer short, high-confidence replies, so set the threshold at 50% rather than 90%.
</details>
...
Third, **a stop condition + event stream** (2 hours). Borrow from Codex's `submit/next_event/turn`. Write every step as a JSON event. Stop predicate: `goal_satisfied || max_iterations || verifier_failed_3_times`.

Without this debugging is misery.
