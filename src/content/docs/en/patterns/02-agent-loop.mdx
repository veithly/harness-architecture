...
:::tip[Key takeaways · 30-second read]
...
## §5 · The four systems' divergences on Agent Loop
...
Between layer 2 (tools) and layer 3 (memory), or between layer 3 and layer 4 (context files). Layers 0-2 are mostly static within a session (identity, mode, tool usage), giving a high cache hit rate. From layer 3 onward content goes dynamic: memory mutates per session, layer 4 changes with cwd, layer 5 changes per turn, layer 6 (output style) stays static but is positioned last by convention.
