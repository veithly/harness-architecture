---
title: 12 · When an Agent Must Ask
sidebar:
  order: 12
description: Contain a generated dangerous action with policy, execution-host controls, and human approval
author: Rick
...
## Permission is decided before execution

<Verdict tldr="Once the model has chosen a dangerous action, which layer can still stop it? We trace policy matching, execution host, human approval, and an independent scanner through Codex, Claude Code, OpenClaw, and Hermes. Enums, matrices, and exit codes are source evidence; without a shared attack benchmark, the recommendations stay conditional on the threat model." />

## First locate the veto boundary
...
## Source evidence: policy, host, and consent

### Codex · Decomposing "permissions" into five orthogonal strongly-typed enums

Permission is not one strictness slider. Codex separates approval modes, action types, and user answers, then derives visible buttons from request context; new enum branches reach compile-time checks.

Its solution is to define each dimension as a strongly-typed enum, forcing every "permission-related decision" in the code to explicitly state which dimension it is deciding on, and forcing the compiler to handle every variant.
...
The Granular tier is where Codex's engineering attitude really shows. It lets users separately configure "which kinds of operations require approval".

There are 5 independent knobs: whether out-of-sandbox operations require approval, whether commands hit by the rules system require approval, whether skill invocations require approval, whether file-permission requests require approval, whether MCP server elicitations require approval.

Each is an independent boolean, so users can compose very specific combinations like "ask for shell commands, do not ask for rule-triggered commands, auto-deny MCP elicitations".

The pragmatic reasoning behind this fine-grained decoupling: different operation types have wildly different risk levels, false-positive rates, and user tolerance. No single "strictness" slider can express that.

The "reviewable action" itself is also a strongly-typed enum, and this is the most distinctive feature of Codex's permission system. It boxes every possible reviewable action into 6 variants, each carrying the fields it needs:
...
What answers can the user give in the approval dialog? Another enum, with 5 values: approve once, approve and propose an execpolicy amendment (so future similar commands auto-approve), approve for the whole session, approve and propose a network-policy amendment, abort.

The last two, "approve and propose a rule amendment", are very clever design: they give the user a shortcut for "approve this and also don't bother me again on the same thing", turning the approval itself into a channel for the permission system to self-evolve.
...
This block of logic reads like a product spec; it uses code to define "in network scenarios show these buttons, in file-permission scenarios show those buttons, in plain command scenarios show another set".

This "context drives button list" approach is the soul of Codex's permission system: the approval dialog's UI is not hard-coded to "always 3 buttons" but dynamically derived from which fields the request carries: carrying network_approval_context shows "approve + approve for session + (optional) modify network policy + deny"; carrying additional_permissions shows only "approve + deny"; a plain command shows "approve + (optional) modify execpolicy + deny".
...
IDE users need to know where a rule came from. Claude Code models user, project, policy, and session sources as traceable fields, while PermissionMode chooses whether the result is allow, deny, or ask.

The reasoning: permission rules in a project, under a team's policy, on a machine, with command-line arguments, inside a session may come from completely different places, and the system must clearly express "which layer this rule comes from", or users have no way to debug "why was this operation allowed/denied".
...
Operators care where a command runs, how it is filtered, and when the user is interrupted. OpenClaw keeps those dimensions separate and includes cwd, agent, session, and environment in the approval binding.

In enterprise agent deployment, ops have to answer: "Which execution environment does this agent run in? Sandbox or host? Is the default safety stance fully-denied, allowlist, or fully-permitted?

When does the user have to be interrupted?" These three questions have completely different answers in different deployment shapes, and a single "strictness" slider cannot express it.
...
Independent combinations across three dimensions theoretically give 27 possibilities (3×3×3); about 4-5 are meaningful.

For example, `host=sandbox / security=allowlist / ask=on-miss` is a very typical production config: "run in a sandbox, auto-allow allowlisted commands, prompt the user when something falls outside the allowlist".

Another, `host=node / security=deny / ask=off`, is a very strict lockdown mode: "run directly on the host, default-deny everything without bothering the user, effectively letting the agent execute nothing", fitting "let the agent think but not act" scenarios.

The biggest engineering value of separating these three dimensions is: **ops can change one dimension without disturbing the other two**.

For example, downgrading production from `sandbox` to `node` (sandbox broken, temporarily degrading) does not require re-deciding "should the default safety stance change"; the other two dimensions stay put.
...
With envHash in the binding, any environment change invalidates the cache and forces a re-approval, plugging the attack.

This "5-tuple binding" also means the same command takes different approval paths in different directories, different agents, different sessions, and different environments.

It is not a simple global allowlist but fine-grained to the specific context.
...
Content scanning can live outside conversation logic. Hermes calls the tirith subprocess, treats its exit code as the verdict, and uses JSON only for findings; fail_open should be documented with the deployment threat model.

Concretely, all content-level permission checks are delegated to an independent subprocess called tirith, and the entire permission system's verdict comes from a single source: the tirith subprocess's exit code.
...
**The first decision is making exit code the sole verdict source**. Subprocess communication typically forces a choice: do you parse JSON from stdout for a "verdict" field, or do you read the process exit code? JSON can carry richer information (findings list, confidence, recommended action), but it also means handling parse failure, JSON truncation, and missing fields. Exit codes have only a few fixed values (0/1/2), which narrows the parsing surface; crashes, timeouts, and unknown codes still need explicit handling. Hermes's choice is **use exit code for verdict, use JSON to enrich findings**. In the cited implementation a broken JSON payload does not change a readable exit code, but that is not a guarantee for every execution failure.
...
**The third decision is auto-download + SHA-256 + optional cosign verification**. tirith is an independent Go binary, and Hermes does not force-depend on it; it detects that tirith is missing and downloads it from GitHub releases to `$HERMES_HOME/bin/tirith` automatically. But "auto-downloading a binary" is itself an attack surface (DNS hijack, man-in-the-middle, compromised GitHub account). Hermes uses HTTPS, a SHA-256 check against the expected digest, and an optional GitHub Actions workflow signature. The digest or signature still depends on a trusted release source; a checksum is not independent provenance proof.

**The fourth decision is keeping installation off the startup critical path.** Tirith downloads in a background thread, avoiding a direct network-and-install dependency during startup. The time saved depends on package size, network, and cache; the source provides no portable benchmark.

Compared with the previous three systems, Hermes provides no fine-grained approval UI and has no concept of "user can temporarily allow". It fully outsources "what counts as dangerous" to tirith, an independent tool.

The benefit of this division is very clean: the agent owns business and UX; tirith owns security decisions; the two communicate over the simplest possible protocol (an exit code).

Tirith can ship new security rules without Hermes changing any code, and Hermes can change conversation logic without touching security rules.

## Invariants shared before execution

Across the four snapshots, three questions recur: mode changes, repeated approvals, and action classification. Treat them as design checks, not universal consensus.
...
**The second is how repeated approvals are handled.** Codex has `ApprovedForSession`, Claude Code has a `session` rule source, and OpenClaw uses a binding as an authorization key; tirith caching needs separate version-level verification. Approval interrupts users, but any cache must bind command, path, host, and session scope rather than remember a global allow.
...
## Combine boundaries by threat model
...
## Choice: put the veto before execution

There is no star rating here. Start by asking who can veto an action, then place policy at the right layer.

| Constraint | Start with | Boundary to make explicit |
| --- | --- | --- |
| A desktop agent needs one-shot and session approvals | Codex typed actions, answers, and contextual buttons | Enum evolution and approval latency |
| Enterprise settings need layered overrides and debugging | Claude Code rule sources, modes, and behaviours | Source precedence must be visible |
| Operators need separate host, filter, and ask controls | OpenClaw execution matrix and binding | Many combinations; document the common ones |
| Security scanning should be independently auditable | Hermes tirith exit-code verdict | fail_open changes the failure risk |

A prompt can express intent; it cannot replace an execution-time policy or host boundary.

## Build the smallest approval loop

Below is a starting checklist from the cited implementations. Complete one auditable allow, deny, and ask loop, then add caching, policy sources, and operator controls from logs.

<BuildRecipe
  title="Build recipe"
  minimal={[
    'Define a 3-tier ApprovalMode enum (borrow from OpenClaw\'s ExecAsk): always-ask / on-miss / never. Three tiers are an auditable starting point; verify coverage against approval logs instead of assuming 90%',
  ...
  advanced={[
    'AskForApproval 5 variants (borrow from Codex): UnlessTrusted / OnRequest / Granular (five flags) / Never + one deprecated compatibility value. The snapshot exposes five choices; validate their fit against approval logs and management policy',
    ...
    'plan mode (borrow from Claude Code): disable side-effect tools such as Edit / Bash / Write so the model plans without acting; use it for PRD, requirements, or code review only when the workflow policy calls for it',
    ...
    'fail_open config fallback (borrow from Hermes): environments that permit checker degradation may choose fail_open=true with a prominent warning; set fail_open=false and enforce it through policy when the checker is a mandatory security gate',
  ]}
  avoid={[
...
## Put decision differences back in context
...
## How to verify approval decisions and rule sources
...
1. **Spawn per call.** Every tool call adds process startup, loading, and IPC work. The source gives no portable 5-50ms or `<1μs` benchmark; measure the two paths on the target platform.
...
`default_available_decisions(ctx)` centralizes "which buttons to show" in the protocol layer. The UI renders whatever list it receives, no logic of its own.

Frontend / TUI / VS Code extension / IDE webview all show the same buttons and never drift.
...
Advance through four scopes: **enum basics → cached binding → multi-source rules → subprocess audit**. Gate each scope on authorization, cache, and audit fixtures before adding the next.
...
**Acceptance gate**: unit tests cover allow, ask, session, and abort decisions; the UI renders the schema returned by `default_available_decisions`.

**Stage 2 · cached binding (defend env-swap)**

**Prerequisite**: Stage 1 action and decision semantics are fixed.
...
**Acceptance gate**: the same binding hits cache, while any cwd, argv, agent, or `env_hash` change misses and is audited.

**Stage 3 · multi-source rules**

**Prerequisite**: user, project, local, CLI, or session rule stacking is a real requirement.
...
**Acceptance gate**: a fixed conflicting-rule fixture produces deterministic precedence, with source and final verdict traceable.

**Stage 4 · subprocess audit (when needed)**

**Prerequisite**: untrusted commands or compliance require an external scanner.
...
**Acceptance gate**: allow, warn, block, and scanner-unavailable verdicts have contract tests, and fail-open/closed behavior follows policy.

**Key takeaways:**

1. **Fix enums first, not strings**: evolution misery starts here
2. **Enable cached binding after fixtures pass**: avoid "same argv prompts twice" UX disaster
...
Models are unreliable. "Plan only, don't act" gets followed by most models, but a non-trivial fraction will pivot to "I see a bug, let me fix it." plan mode pulls every side-effect tool from the toolbox at the framework level; the model can't act even if it wants to.

Hard constraint beats prompting.
...
- 100-1000 turns: a threshold such as < 5% denial rate can be a guarded-trial input; calibrate it from your logs and do not auto-pass without audit and rollback controls
...
If download sits on the main path, a 30-second first-launch wait is awful. Hermes spawns a background thread to download without blocking input.

Without tirith installed, the first tool call falls back to fail_open behavior (or blocks waiting for the download, per config).
...
<details>
<summary>**Q10 · Open-ended**: Design a permission framework from a threat model and state every grant scope.</summary>
...
- **Layers 1-3 · typed decision core**: fix the mode, action, decision, and button-inference contracts first. **Gate**: unit tests cover allow, ask, and abort; the UI only renders buttons returned by the schema.
- **Layers 4-5 · binding and rule sources**: add only when session reuse or user/project/CLI rule stacking is required. **Prerequisite**: the binding includes cwd, argv, agent, and `env_hash`; **gate**: hit/miss, env-swap, and rule-precedence fixtures agree with the audit record.
- **Layers 6-7 · subprocess and supply-chain audit**: add only for untrusted commands or enterprise compliance. **Prerequisite**: fail-open/closed policy is explicit; **gate**: allow, warn, block, and auditor-unavailable verdicts each have contract tests.

**Key decisions:**

1. **Prefer enums, never strings**
...
**Source route**: Codex shows typed approvals, Claude Code rule sources, OpenClaw hosts and bindings, and Hermes an external verdict. Their grant semantics differ, so do not combine them verbatim; select by threat model and add contract tests.
</details>
