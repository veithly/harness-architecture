...
Codex is OpenAI's first-party coding agent. Its core philosophy is straightforward: model self-evaluation cannot be trusted, so build "task done" into a machine-verifiable thing instead. The Rust core (`codex-rs` workspace) shows this judgement at every layer.
...
All four write the prompt in markdown (Codex one big `.md`, Claude Code string concatenation in `constants/prompts.ts`, OpenClaw `buildXxxSection` returns strings, Hermes `SOUL.md` is markdown). But tool calls all use JSON (Anthropic `tool_use` / OpenAI `tool_calls`), because tool calling demands structured fidelity.
