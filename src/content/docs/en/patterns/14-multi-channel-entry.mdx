...
## Four entry topologies

<details>
<summary>Before reading (optional)</summary>

- **One agent core, many callers**: terminals, CI scripts, IDE plugins, chat platforms, and webhooks all need different lifecycles and protocols.
- **Four splits**: Codex separates binaries and bridges IDE to daemon via hidden `codex stdio-to-uds`; Claude Code is one binary with a `--bare` mode for CI/SDK; OpenClaw flattens 19 sub-CLIs as RPC clients to one gateway daemon; Hermes folds 17 platform adapters behind one Python ABC with 6 required methods.
- **A stateless stdio-to-socket bridge determines upgradability**: the daemon can restart or upgrade without restarting the IDE, because the bridge holds no state.
- **Match the pattern to the agent type**: developer tools borrow Codex / Claude Code; ChatOps reuses Hermes's adapter base class; open ecosystems reuse OpenClaw's channels-plugins.
...
## How four systems connect multiple channels

### Codex · Make every calling mode a separate subcommand, then bridge IDE and daemon over a Unix domain socket

Codex's reasoning is clear: it has to serve callers with wildly different needs. A developer running bare in the terminal wants a full ratatui-rendered TUI. A CI script wants something that takes a prompt and produces JSON before exiting.
...
Claude Code's trade-off is the exact opposite of Codex. It assumes its users are developers who install with one `npm install -g @anthropic-ai/claude-code`; what it most wants to avoid is the user needing to remember "oh, you have to install another binary to get MCP mode".
...
OpenClaw's reasoning differs from both of the above. The scenario it serves is much more complex than an IDE tool.
...
Hermes's trade-off contrasts sharply with OpenClaw. Its goal is to be a ChatOps agent where "one pip install plus boot, you immediately have access to all 17 chat platforms", so it bundles every platform adapter into the main repo and runs them all inside the same gateway process.
...
## What multi-channel entry already agrees on
...
**The fourth is that the protocols spoken are open standards: nobody invents private protocols**. MCP (used by Codex, Claude, Hermes), ACP (OpenClaw's bet), Telegram Bot API, Slack Bolt, Discord Gateway, IMAP/SMTP are all public. Nobody has built "a proprietary agent communication protocol", because public protocols come with existing SDKs and tooling, and private protocols would lock the ecosystem out, which would in turn limit the agent's own reachable scenarios.

## Choose subcommands, adapters, or separate packages
...
## My take: channels should only translate
...
## Four entry topologies side by side
...
## Follow channel adapters through source
...
Hermes positions as "ChatOps-first." HTTP API and Webhook are secondary entries, no need to architect around them. Reusing platform adapter keeps code small, maintenance simple.
