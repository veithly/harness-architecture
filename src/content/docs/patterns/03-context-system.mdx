...
- **Context 系统决定模型每次推理能看到什么**：同模型同工具，换一套 context 系统的表现差距比换模型还大。
- **组装方式分歧最大**：Codex 用 24 个 ContextualUserFragment 强类型 fragment 加 START/END marker，Claude Code 用 SYSTEM_PROMPT_DYNAMIC_BOUNDARY 切缓存边界，OpenClaw 用三档 PromptMode。
- **静态与动态的切分决定 prompt cache 命中率**：静态可缓存，动态每 turn 重算。
- **最大的坑是外部文件**：AGENTS.md、.cursorrules 不可默认信任，注入前要做 prompt injection 扫描（Hermes 的 _scan_context_content 命中即替换 [BLOCKED]）。
...
这种「小 markdown 文件加 fragment 类型」组合让 prompt 改动可以做精细 diff（哪个文件被改了一行 git log 一查就知道），调试时也更容易复现（每个 fragment 都能单独 render 出来检查）。这套设计把「context = 标记好类型的消息数组」做得很彻底，是四家里 context 工程细节最多的。代价是代码量大，24 个 fragment 都要写 trait impl 加模板文件，比直接字符串拼接重很多。

### Claude Code · 字符串数组加显式 cache boundary，把缓存命中率做得很深
...
切成静态前半加动态后半让前半命中 cache，成本可以下降 5-10 倍。
...
用一坨字符串拼接得维护三份不同的 prompt 文件，用强类型 fragment 又太重，最好的折中是「模块化函数加模式开关」。
...
**想适配多身份（主 agent、subagent、外部调用方）**：参考 OpenClaw 的 buildXxxSection 加 PromptMode 三档路线。模块化函数加三档模式让同一套代码同时服务三种身份，扩展新身份只需要加一个 PromptMode 值。ctx 参数贯穿装配链让运行时状态注入有统一接口。但它不显式切 cache（主 agent 和 subagent 走同一个 prompt 函数时 cache 会污染）。适合多通道、多角色 agent。
...
<ScoreCard
  rows={[
    { system: 'Codex', value: '★★★★☆', reason: '24 个 fragment 把「context = 标类型的消息」做得很深，压缩时可按 marker 还原，prompt 文件全入仓可 diff', risk: 'fragment 加新种类要改 mod.rs，扩展性不如纯函数。缺少注入前安全扫描' },
