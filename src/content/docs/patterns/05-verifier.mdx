...
- **模型说「我做完了」基本不能信**：loop 完全靠模型自评收尾，必然出现「bug 没修但 agent 说改完了」的灾难。
- **覆盖度分歧最大**：Codex 4 个硬 verifier 串到底（execpolicy 三态、goals.rs 收敛检测），Claude Code 只有 TOKEN_BUDGET 软算法，OpenClaw 靠 tool-loop-detection 4 种 detector，Hermes 靠 insights 跨会话累积。
- **verifier 层数决定翻车概率**：硬、软、放任型三层都得有，缺一翻车。
- **自己动手第一优先**：先接外部客观信号（exit code、patch 合法性、tests），别指望模型自评。
...
Codex 把这些客观信号全部串起来，loop 完全不依赖模型自评。
...
所以 Claude Code 选择把软 verifier 做得很深：用 token 收敛速率做主要判退信号，便宜（不用跑外部命令）、快速（每轮就能算）、对场景普适（任何任务都能算 token 增量）。
...
这是 Claude Code 在 verifier 上明显的盲点：对于需要严格保证「测试通过才能 PR」的工作流，Claude Code 不适合直接用。
...
最好的办法是把 verifier 做成中间件接口，让具体的检查逻辑由使用者自己注册。
...
Hermes 在 verifier 设计上的出发点是：长跑助理（陪伴用户几个月几年）的 verifier 不该是「单次 loop 内一定要严格判对错」。这种严格性会让 agent 太死板没法处理多样性场景，反而损害用户体验。正确的方式是把 verifier 摊到时间轴上：单次 loop 不严格但每次 loop 结束后做事后分析写回 memory，下次类似任务 prefetch 出来注入 context，agent 就会越来越聪明。
...
**第三，软 verifier 用预算和启发式（便宜但够用）**：token 增量、重试次数、调用模式哈希，这些信号成本低（不用跑外部命令）但能拦下大部分浪费。Claude Code 把软 verifier 做得很深，60 行算法覆盖主要场景。
