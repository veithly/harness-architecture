---
title: 12 · Agent 什么时候必须问人
sidebar:
  order: 12
description: 在危险动作已经生成后，用策略、执行宿主和人工确认缩小副作用
author: Rick
...
<Verdict tldr="模型已经决定执行危险动作时，哪一层还能拦住它？本文把一次执行拆成策略匹配、运行宿主、人工确认和独立扫描四个边界，追踪 Codex、Claude Code、OpenClaw、Hermes 在何处做决定。它们的枚举、矩阵和 exit code 是源码证据；没有统一攻击向量的本站实验，所以选型结论按 threat model 写成条件句。" />
...
权限不是一根“严格度”滑杆。Codex 把审批模式、操作类型和用户答案分开建模，再由请求上下文决定可见按钮；源码中的枚举让新增分支进入编译检查。
...
这 5 种模式的含义各自很具体：UnlessTrusted（也叫「untrusted」模式）只自动放行那些被静态判定为「绝对安全」的只读命令（比如 `ls`、`cat` 这种），其他一律问。OnFailure 是一个已经被标记为废弃的模式（理论上是「全部自动放行，靠沙箱兜底」，但沙箱不是万能的，所以现在不推荐用）。OnRequest 是默认模式，让模型自己判断什么时候该问用户。Granular 是把决策权进一步切碎成 5 个独立旋钮交给用户配（下面会讲）。

Never 顾名思义就是永远不问，适合 CI 等明确希望自动化的场景。

Granular 这一档是实际展现 Codex 工程态度的地方：它让用户可以分别配置「哪类操作要问」。具体来说有 5 个独立的旋钮：要不要审批沙箱外的操作、要不要审批被规则系统命中的命令、要不要审批 skill 的调用、要不要审批文件权限请求、要不要审批 MCP 服务的弹出式询问。每个旋钮都是独立的布尔值，所以用户可以拼出「shell 命令要问、但规则触发的不用问、MCP 自动拒」这种很具体的组合。

这种细粒度的解耦背后是一个很务实的判断：不同类型操作的风险等级、误报率、用户的容忍度都不一样，强行用一个统一的「严格度」滑杆没法表达这种差异。
...
IDE 用户需要知道规则从哪来。Claude Code 把 user、project、policy、session 等来源建模为可追踪字段，PermissionMode 再决定 allow、deny 或 ask。
...
运维关心命令在哪跑、过滤多严、何时打扰用户。OpenClaw 把宿主、安全级别和询问策略拆成独立维度，并把 cwd、agent、session、环境纳入审批 binding。
...
**ExecSecurity** 回答「这个执行环境的默认安全策略是什么」：选 `deny` 表示默认禁止一切（需要显式 allowlist 才能跑）、选 `allowlist` 表示只允许 allowlist 里的命令、选 `full` 表示默认放行一切（除非显式 denylist）。

**ExecAsk** 回答「什么时候打扰用户」：选 `off` 表示从不问、选 `on-miss` 表示只有「想跑但 allowlist 不允许」时才问、选 `always` 表示每次执行都问。

三个维度独立组合理论上有 27 种可能（3 乘 3 乘 3），实际有意义的组合大约 4 到 5 个。比如 `host=sandbox / security=allowlist / ask=on-miss` 是一个典型的生产配置：「在沙箱里跑，白名单内的命令直接放行，没命中白名单就弹窗问用户」。

再比如 `host=node / security=deny / ask=off` 是一个很严格的封锁模式：「直接在宿主跑，但默认全禁不打扰，让 agent 完全无法执行任何命令」，适合「只让 agent 思考不让动手」的纯规划场景。

把这三维拆开的最大工程价值是：**运维改其中一维不会影响另外两维**。比如生产环境从 `sandbox` 切到 `node`（沙箱出问题暂时降级）不需要重新决定「默认安全策略要不要变」，后两维不动。

权限决策一旦给出，OpenClaw 不会让用户每次都重新审批同一个命令。它用一个 5 元组作为审批的单一标识来做缓存：
...
这 5 个字段一起决定「两次执行是不是同一个审批」。其中 `envHash` 这一项是特别值得讲的：它把环境变量的哈希值也纳入审批 key 的一部分，这是为了防一种微妙的攻击：用户在某个 PATH 配置下批准了 `python script.py`（这时 `python` 指的是系统 Python），攻击者悄悄改了 PATH 让 `python` 指向恶意脚本，下次 agent 跑同一个命令，如果不考虑环境变量，缓存命中后直接放行恶意脚本就执行了。

把 envHash 纳入 binding 后，环境变量一变缓存自动 miss，需要重新审批，攻击就被堵住了。
...
内容扫描可以独立于对话逻辑。Hermes 调 tirith 子进程，以 exit code 定 verdict，JSON 只补充 finding；fail_open 的选择要跟部署风险一起写。

<SourceBlock path="REF/hermes-agent/tools/tirith_security.py" lines="1-25" system="hermes" summary="tirith 是独立 Go 二进制，exit code 是单一 verdict source-of-truth">
...
**第一个决策是把 exit code 作为单一的 verdict 来源**。子进程通信通常会面对一个选择：是看 stdout 里返回的 JSON 解析「verdict」字段呢，还是看进程的 exit code？JSON 可以携带更丰富的信息（finding 列表、置信度、推荐操作），但也意味着解析失败、JSON 截断、字段缺失这些情况都需要处理。exit code 只有几个固定值（0、1、2），解析面较小，但进程崩溃、超时和未知码仍然需要单独处理。Hermes 的取舍是**用 exit code 决定 verdict，用 JSON 丰富 findings**：在当前实现里，即使 tirith 的 JSON 损坏，verdict 仍可由退出码读取；这不是对所有执行失败的保证。JSON 只用来在 verdict 已定的前提下，给用户、log 看的「哪几条规则被命中、命中位置在哪、置信度多高」这种辅助信息。
...
**第三个决策是自动下载加 SHA-256 加可选 cosign 验签**。tirith 是一个独立的 Go 二进制，用户如果没装 Hermes 不会强制依赖它：而是检测到没有时从 GitHub releases 自动下载到 `$HERMES_HOME/bin/tirith`。但「自动下载二进制」本身是一个攻击面（DNS 劫持、man-in-the-middle、GitHub 账户被盗）。Hermes 用三层防御来保护这个流程：HTTPS 保护传输、SHA-256 校验检查文件是否匹配预期摘要、如果系统上装了 cosign 还会验证 GitHub Actions 工作流签名。摘要或签名本身仍依赖可信的发布来源；不能把 checksum 当成独立的来源证明。

**第四个决策是后台安装不阻塞启动**。tirith 的下载放在后台线程里，避免把网络与安装路径直接加到 agent 启动关键路径；具体节省多少时间取决于包体、网络和缓存，源码没有提供可迁移的基准。
...
把四个源码快照放在一起，会反复遇到模式切换、重复审批和动作分类三个问题。它们是设计检查项，不是“四家共识”或行业必修课。

**第一件事是需要有「模式」这个粗粒度开关**。无论权限规则有多细，用户都需要一个一键切换的「模式」概念来快速调整 agent 的默认行为：「我现在很赶时间，给我 YOLO 模式」或者「我在生产环境，给我最严格的模式」。Codex 用 5 种 AskForApproval、Claude Code 用 5 种 PermissionMode、OpenClaw 用 3 种 ExecAsk、Hermes 用 fail_open 这个二元开关，颗粒度不同但都在做同一件事：让用户不需要逐条规则配也能快速切换。

**第二件事是如何处理重复审批**。Codex 有 `ApprovedForSession`，Claude Code 有 `session` 规则来源，OpenClaw 用 binding 作为授权键；tirith 是否以及怎样缓存需要按其独立版本核验。审批会打断用户，但缓存必须绑定命令、路径、宿主和会话范围，不能只做一个全局“记住允许”。

**第三件事是需要把"可审批的操作"分类**。不同类型的操作风险等级、UI 要求、缓存策略都不一样，硬塞到一个统一的"审批"概念里会失去精度。Codex 用 6 种 GuardianAssessmentAction 强类型分类、Claude Code 通过 toolName 路由不同的审批 UI、OpenClaw 通过 ExecHost 区分 3 种执行宿主、Hermes 让 tirith 在 findings 里给操作分类。背后的共识是：审批不应该是个一刀切的概念。
...
这里不做星级评分。权限设计先回答“谁能否决”，再决定策略要放在哪一层。

| 约束 | 可先读 | 需要明确的边界 |
| --- | --- | --- |
| 桌面 Agent 要让用户逐次或按会话批准 | Codex 的 typed actions、answers 和动态按钮 | 枚举演化与审批延迟 |
| 企业设置需要分层覆盖和排错 | Claude Code 的 rule sources、modes 和 behaviors | 来源优先级需要可见 |
| 运维要独立调整宿主、过滤和询问时机 | OpenClaw 的三维执行矩阵与 binding | 组合多，需给常用配置 |
| 安全扫描要独立升级、可审计 | Hermes 的 tirith exit-code verdict | fail_open 选择决定故障时的风险 |

Prompt 只能提供意图，不能替代执行前的策略和宿主隔离。

## 从最小审批闭环开始

下面是从引用实现整理出的起步清单。先完成一次可审计的 allow、deny、ask 闭环，再按日志添加缓存、策略来源和运维配置。

<BuildRecipe
  title="复刻方案"
  minimal={[
    '定义 ApprovalMode 枚举 3 档（参考 OpenClaw 的 ExecAsk）：always-ask（每次都问）、on-miss（白名单未命中才问）、never（不问，按底层规则决定）。3 档是一个可审计的起点；是否覆盖你的场景要用审批日志验证',
  ...
  advanced={[
    'AskForApproval 5 档（参考 Codex）：UnlessTrusted（除非命令是 trusted 否则问）、OnRequest（模型主动 request_permissions 才问）、Granular（按 5 个 flag 单独配）、Never 加一个 deprecated 兼容档。源码暴露了五种选择；是否适合企业策略要按审批日志和管理面需求验证',
    ...
    'plan 模式（参考 Claude Code）：可以关闭 Edit、Bash、Write 等副作用工具，让模型只规划不执行；是否用于 PRD、需求讨论或 code review 取决于工作流策略',
    '内容安全外包子进程（参考 Hermes 的 tirith 模式）：把内容安全检查独立到子进程，exit code 是真理之源（不在乎子进程崩、输出 JSON 错），JSON 只丰富 findings 但不影响 verdict',
    'fail_open 配置兜底（参考 Hermes）：允许检查器降级的环境可 fail_open=true 加显著告警；把检查器当强制安全门槛的策略才设 fail_open=false，并由 policy 强制',
  ]}
  avoid={[
    ...
    '别忘了 cosign、SHA-256 供应链验证：自动下载的安全检查 binary 也是攻击面（attacker 替换 cdn 上的 binary）。下载后需要验签加校验 SHA',
  ]}
/>
...
<SourceTrail
  ...
  entries={[
    ...
    { system: 'Claude Code', path: 'REF/claude-code-2.1.88-expanded/src/types/permissions.ts', lines: '1-100', desc: 'PermissionMode / PermissionBehavior / PermissionRule 较全定义' },
...
agent 经常切换 env（启用 venv、source .env、export 临时变量）。每次都重新审批 UX 太差。envHash 让"实际影响 lookup 的 env"（PATH / LD_LIBRARY_PATH / 等）触发 reapproval，其他 env 变化（PS1 / TERM）忽略。
...
1. **每次都要 spawn 子进程。** 每个 tool call 都会多一次进程启动、加载和 IPC 路径；源码没有给出可迁移的 5-50ms 或 `<1μs` 基准，应在目标平台测量。
...
按 **enum 基础 → 缓存 binding → 多源规则 → 子进程审计** 四个 scope 推进；每层先通过授权、缓存和审计 fixture，再决定是否增加下一层。
...
**验收门槛**：allow、ask、session 和 abort 的决策单测覆盖，UI 按 `default_available_decisions` 返回的 schema 渲染。

**阶段 2 · 缓存 binding（防 env-swap）**

**前置**：阶段 1 的 action 和 decision 语义已固定。
...
**验收门槛**：相同 binding 命中缓存，cwd、argv、agent 或 `env_hash` 任一变化都会 miss，并写入审计记录。

**阶段 3 · 多源规则叠加**

**前置**：确实存在 user、project、local、CLI 或 session 规则的叠加需求。
...
**验收门槛**：固定冲突规则 fixture 得到确定优先级，来源和最终 verdict 可追踪。

**阶段 4 · 子进程审计（按需）**

**前置**：执行不受信任命令或合规要求需要外部 scanner。
...
**验收门槛**：allow、warn、block 和 scanner 不可用四种 verdict 都有契约测试，fail-open/closed 行为与策略一致。

**关键经验**：

  1. **先固定 enum，不要 string**：演化痛苦的根源
  2. **binding 缓存通过 fixture 后再启用**：防止「同样 argv 跑两次都弹窗」UX 灾难
...
- 100-1000 turn：把拒绝率低于 5% 的命令类型列为候选，先在审计和回滚策略下做灰度，不要直接自动放行
...
<details>
<summary>**Q10 · 开放**：按威胁模型设计权限框架，并说明每个授权范围。</summary>
...
- **Layer 1-3 · typed decision core**：先固定 mode、action、decision 和按钮推断契约。**验收**：枚举/默认决策的单测覆盖 allow、ask、abort，UI 只渲染 schema 返回的按钮。
- **Layer 4-5 · binding 与多源规则**：只有在需要 session 复用或 user/project/CLI 规则叠加时增加。**前置**：binding 包含 cwd、argv、agent 和 `env_hash`；**验收**：hit/miss、env-swap 和规则优先级 fixture 与审计记录一致。
- **Layer 6-7 · 子进程与供应链审计**：仅在执行不受信任命令或企业合规要求时增加。**前置**：明确 fail-open/closed 策略；**验收**：allow、warn、block、审计工具不可用四种 verdict 都有契约测试。
...
**源码路线**：Codex 展示 typed approval，Claude Code 展示规则来源，OpenClaw 展示宿主与 binding，Hermes 展示外部 verdict。它们的授权语义不同，不能直接拼成一个框架；应按威胁模型选择并补契约测试。
</details>
