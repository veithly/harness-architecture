...
- **核心问题**：审批要答四问：什么时候弹窗、问什么、答复保留多久、权限不够时怎么升级；四家复杂度差整整一个数量级。
- **分歧最大点**：Codex 全强类型枚举（AskForApproval 5 种、GuardianAssessmentAction 6 种、ReviewDecision 5 种）、Claude Code 是 PermissionMode 5 种加 8 种规则来源、OpenClaw 用 ExecHost 乘 ExecSecurity 乘 ExecAsk 的 27 组合矩阵、Hermes 全部委托 tirith 子进程 exit code（0 允许、1 阻止、2 警告）。
- **工程判断**：枚举化决定可维护性：策略、操作、用户答案全部强类型，是做严肃 agent 的底线。
- **动手要点**：先想清楚答复的生命周期：一次有效、整个会话、永久（Approved 对比 ApprovedForSession 对比 ApprovedExecpolicyAmendment），别只做布尔放行。
...
Granular 这一档是真正展现 Codex 工程态度的地方：它让用户可以分别配置「哪类操作要问」。具体来说有 5 个独立的旋钮：要不要审批沙箱外的操作、要不要审批被规则系统命中的命令、要不要审批 skill 的调用、要不要审批文件权限请求、要不要审批 MCP 服务的弹出式询问。每个旋钮都是独立的布尔值，所以用户可以拼出「shell 命令要问、但规则触发的不用问、MCP 自动拒」这种很具体的组合。
...
把 envHash 纳入 binding 后，环境变量一变缓存自动 miss，必须重新审批，攻击就被堵住了。
