...
特别值得讲的是两个带 `Managed` 前缀的配置，`allowManagedDomainsOnly` 和 `allowManagedReadPathsOnly`。它们的语义是："开了这个之后忽略所有用户层配置，只信 policySettings"。这是企业 IT 管理员的"我说了算"心态：管理员开了之后，无论用户在 `userSettings`、`projectSettings`、`localSettings`、`cliArg` 里怎么放行，都不生效；只有管理员在 `policySettings` 里下发的规则才算数。这种"管理面凌驾用户面"的设计是企业部署的基本盘。
